...
export const Film = () => {
    ...
    const items = [
        ...
        {caption: 'Slogan', value: slogan, condition: slogan},
    ]

    const tabs = [
        {txt: 'Description', content: <p className={styles.desc}>{description}</p>, condition: description?.length},
        {txt: 'Actors', content: <Actors filmId={data?.kinopoiskId}/>},
        {txt: 'Facts', content: <Facts filmId={data?.kinopoiskId}/>},
    ]

    const movieYear = year && `(${year})`;
    const movieTitle = nameRu ? nameRu : isLoading ? 'Loading' : 'Untitled';
